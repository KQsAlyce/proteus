...
namespace ns3 {
	...
	void ProteusRouting::SetConstants(Time probeInterval, Time dreTime, Time onewayRttlow, double linkThreshold, uint32_t nsample) {
	...
	std::vector<uint32_t> ProteusRouting::GetPathSet(uint32_t dstToRId, uint32_t nPath) {
		...
		while (!nonCongested_path.empty()) {
			...
			uint32_t rand_idx = rand() % idx.size();
	...
	bool ProteusRouting::getBetterPath(uint64_t now, uint64_t lastTxTime, uint32_t dstToRId, uint32_t& betterPath) {
		uint32_t selectedPath = betterPath;
		...
		for (it = m_proteusPathInfoTable.find(dstToRId)->second.begin(); it != m_proteusPathInfoTable.find(dstToRId)->second.end(); it++) {
			...
			// if (pathStatus==true && it->second.oneway_rtt < ((selectedPathRtt>6000)?selectedPathRtt-6000:1)) {
			if (pathStatus==true && it->second.oneway_rtt+6000 < selectedPathRtt) {  // 在左边加更方便....
